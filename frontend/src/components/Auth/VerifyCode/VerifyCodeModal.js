...
export default function VerifyCodeModal({ open = false, onClose }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok && data.code === 200) {
                ...
                if (
                    data.code === 1010 ||
                    (data.message && data.message.includes('OTP'))
                ) {
    ...
    return (
        <div className={cx('verify-container')}>
            <Button onClick={handleBack} className={cx('back-button')}>
                ←
            </Button>
                    ...
                    <div className={cx('resend-section')}>
                        {seconds > 0 ? (
                            <span className={cx('countdown-text')}>
                                Gửi lại sau{' '}
                                <span className={cx('countdown-time')}>{`00:${seconds
                                    .toString()
                                    .padStart(2, '0')}`}</span>
                            </span>
                        ) : (
