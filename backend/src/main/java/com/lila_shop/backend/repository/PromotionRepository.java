...
public interface PromotionRepository extends JpaRepository<Promotion, String> {
        ...
        @Query("SELECT p FROM Promotion p JOIN p.categoryApply c "
                        + "WHERE c.id IN :categoryIds AND p.status = 'APPROVED' AND p.isActive = true "
                        + "AND (p.expiryDate IS NULL OR p.expiryDate >= :today)")
        List<Promotion> findActiveByCategoryIds(@Param("categoryIds") java.util.Collection<String> categoryIds,
        ...
        List<Promotion> findExpiredPromotions(@Param("today") LocalDate today,
                        @Param("expiredStatus") PromotionStatus expiredStatus);
